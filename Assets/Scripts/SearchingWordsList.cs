...
public class SearchingWordsList : MonoBehaviour
{
   ...
   private void Start()
    ...
    // preventing counting more rows than there already is 
    ...
    private bool ShouldScaleDown(Vector3 targetScale)
    {
        ...
        var paremtRect = this.GetComponent<RectTransform>();
        ...
        if (totalSquareHeight > paremtRect.rect.height)
        {
            while (totalSquareHeight > paremtRect.rect.height) {
    ...
    private Vector2 GetFirstSquarePosition()
    {
        ...
        var sqaureSize = new Vector2(0f, 0f);

        sqaureSize.x = squareRect.rect.width * squareRect.transform.localScale.x + offset;
        sqaureSize.y = squareRect.rect.height * squareRect.transform.localScale.y + offset;

        // make sure they are in the center
        var shiftBy = (parentRect.rect.width - (sqaureSize.x * _columns)) / 2;

        startPosition.x = ((parentRect.rect.width - sqaureSize.x) / 2) * (-1);
